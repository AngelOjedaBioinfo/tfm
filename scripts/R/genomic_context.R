...
cds_dmr5hmC    <- count_lines(paste0(ann, "dmr.5hmC.in_exons.bed"))  # aproximacion
...
context_data <- data.frame(
  context = c(rep(c("CDS", "UTR", "Intron", "Intergenic"), 2),
  ...
  n = c(
    ...
    cds_dmr5hmC, intron_dmr5hmC, inter_dmr5hmC
  )
)
...
p <- ggplot(context_data, aes(x=type, y=percentage, fill=context)) +
  ...
  labs(x="", y="Percentage (%)", fill="Genomic context",
